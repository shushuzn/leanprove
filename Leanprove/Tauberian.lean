...
noncomputable def ntermReal (f : ℕ → ℝ) (σ : ℝ) (n : ℕ) : ℝ := if n = 0 then 0 else f n / (n : ℝ) ^ σ

/-- `termReal f s n` is the complex term of the Dirichlet series of real-valued `f` at `s`. -/
noncomputable def termReal (f : ℕ → ℝ) (s : ℂ) (n : ℕ) : ℂ := if n = 0 then 0 else (f n : ℂ) / n ^ s

/-- `termReal` agrees with `LSeries.term` applied to the complexified `f`. -/
lemma termReal_eq_LSeries_term (f : ℕ → ℝ) (s : ℂ) (n : ℕ) :
    termReal f s n = LSeries.term (fun n => (f n : ℂ)) s n := by
  by_cases h : n = 0
  · simp [h, termReal, LSeries.term]
  · simp only [termReal, LSeries.term, h, ↓reduceIte, ne_eq, not_false_eq_true, h]

/-- The Dirichlet series of `f` equals the sum of `termReal f s n`. -/
theorem LSeries_eq_tsum_termReal (f : ℕ → ℝ) (s : ℂ) :
    LSeries (fun n => (f n : ℂ)) s = ∑' n, termReal f s n := by
  simp [LSeries, LSeries.term, termReal_eq_LSeries_term, tsum_congr fun n => ?_]

/-! #### Wiener-Ikehara Tauberian Theorem -/

/-- von Mangoldt satisfies Chebyshev bound: ∑_{n<N} Λ(n) ≤ C*N -/
theorem vonMangoldt_cheby : chebyReal vonMangoldt := by
...
theorem WienerIkeharaTheorem
  ...
  have hf' : ∀ (σ' : ℝ), 1 < σ' → Summable (nterm (fun n => (f n : ℂ)) σ') := by
    intro σ' hσ'
    have := hf_sum σ' hσ'
    refine Summable.congr this (fun n => ?_)
    by_cases hn : n = 0
    · simp [hn, nterm, zero_rpow (ne_of_gt (by linarith : (0 : ℝ) < σ'))]
    · simp only [nterm, hn, ↓reduceIte]
      rw [Complex.norm_of_nonneg (hf_pos n)]
  have hcheby' : cheby (fun n => (f n : ℂ)) := by
    obtain ⟨C, hC⟩ := hcheby
    refine ⟨C, fun n => ?_⟩
    change cumsum (fun i => ‖(f i : ℂ)‖) n ≤ C * n
    have hnorm : (fun i => ‖(f i : ℂ)‖) = f := by
      ext i; simp [norm_of_nonneg (hf_pos i)]
    rw [hnorm]
    exact hC n
  have hG' : Set.EqOn G (fun s ↦ LSeries (fun n : ℕ ↦ (f n : ℂ)) s - (A : ℂ) / (s - 1)) {s | 1 < s.re} :=
    hG_eq
  exact WienerIkeharaTheorem' hpos hf' hcheby' hG_cont hG'
