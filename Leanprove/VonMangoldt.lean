...
lemma log_lt_two_sqrt {p : ℝ} (hp : 1 ≤ p) : Real.log p < 2 * Real.sqrt p := by
  ...
  calc
    Real.log p = Real.log ((Real.sqrt p) ^ 2) := by rw [Real.sq_sqrt (by linarith)]
    _ = 2 * Real.log (Real.sqrt p) := by rw [Real.log_pow, Nat.cast_ofNat]
    _ ≤ 2 * (Real.sqrt p - 1) := by gcongr
...
lemma sqrt_div_sq_eq_rpow (p : ℕ) (hp : 0 < p) : Real.sqrt (p : ℝ) / ((p : ℝ) ^ (2 : ℝ)) = (p : ℝ) ^ (-3/2 : ℝ) := by
  ...
  have h_sub : (1/2 : ℝ) - (2 : ℝ) = (-3/2 : ℝ) := by ring
  calc
    Real.sqrt (p : ℝ) / ((p : ℝ) ^ (2 : ℝ)) = ((p : ℝ) ^ (1/2 : ℝ)) / ((p : ℝ) ^ (2 : ℝ)) := by rw [Real.sqrt_eq_rpow]
    _ = (p : ℝ) ^ ((1/2 : ℝ) - (2 : ℝ)) := by rw [← Real.rpow_sub hp_pos]
    _ = (p : ℝ) ^ (-3/2 : ℝ) := by rw [h_sub]
...
lemma log_div_sq_bound_le (p : ℕ) (hp : 2 ≤ p) : Real.log (p : ℝ) / ((p : ℝ) ^ (2 : ℝ)) ≤ 2 * ((p : ℝ) ^ (-3/2 : ℝ)) := by
  ...
  have h1 : Real.log (p : ℝ) / ((p : ℝ) ^ (2 : ℝ)) < (2 * Real.sqrt (p : ℝ)) / ((p : ℝ) ^ (2 : ℝ)) := by
    apply (div_lt_div_right h_sq_pos).mpr; exact h_log
  have h2 : (2 * Real.sqrt (p : ℝ)) / ((p : ℝ) ^ (2 : ℝ)) = 2 * ((p : ℝ) ^ (-3/2 : ℝ)) := by
    calc
      (2 * Real.sqrt (p : ℝ)) / ((p : ℝ) ^ (2 : ℝ)) = 2 * (Real.sqrt (p : ℝ) / ((p : ℝ) ^ (2 : ℝ))) := by ring
      _ = 2 * ((p : ℝ) ^ (-3/2 : ℝ)) := by rw [sqrt_div_sq_eq_rpow p (by omega : 0 < p)]
  rw [h2] at h1; linarith

lemma pow_le_pow_of_nonneg_left {a b : ℝ} (ha : 0 ≤ a) (hab : a ≤ b) (n : ℕ) : a ^ n ≤ b ^ n := by
  induction'' n with k ih
  · simp
  · rw [pow_succ, pow_succ, mul_comm (a ^ k), mul_comm (b ^ k)]
    have hb : 0 ≤ b := le_trans ha hab
    refine mul_le_mul hab ih (pow_nonneg ha k) hb
...
lemma range_sum_le_tsum_of_nonneg (f : ℕ → ℝ) (h_nonneg : ∀ n, 0 ≤ f n) (h_summable : Summable f) (N : ℕ) :
  ...
  have h_mono : ∀ n m, n ≤ m → s n ≤ s m := by
    intro n m hnm
    obtain ⟨k, hk⟩ := Nat.exists_eq_add_of_le hnm; rw [hk]
    calc
      s n = ∑ i ∈ Finset.range n, f i := rfl
      ...
      _ = s (n + k) := rfl
  by_contra! h
  have h_eps : s N - a > 0 := by linarith
  ...
  have h_real_dist : dist (s n) a = |s n - a| := Real.dist_eq (s n) a
  rw [h_real_dist] at hn_dist
...
lemma sum_half_pow_range_le_two (M : ℕ) : ∑ j ∈ Finset.range M, ((1 : ℝ) / 2) ^ j ≤ 2 := by
  have h_geom : ∑ j ∈ Finset.range M, ((1 : ℝ) / 2) ^ j = 2 * (1 - ((1 : ℝ) / 2) ^ M) := by
    have h := geom_sum_eq (by norm_num : (1/2 : ℝ) ≠ 1) M; rw [h]; ring
  rw [h_geom]; have h_nonneg : 0 ≤ ((1 : ℝ) / 2) ^ M := by positivity; nlinarith

lemma pow_div_pow_bound (p : ℕ) (hp : 2 ≤ p) (k : ℕ) (hk : 2 ≤ k) :
    (1 : ℝ) / ((p : ℝ) ^ k) ≤ ((1 : ℝ) / 2) ^ (k - 2) * (1 / ((p : ℝ) ^ 2)) := by
  have hp_pos : (p : ℝ) > 0 := by exact_mod_cast (show 0 < p from by omega)
  calc
    (1 : ℝ) / ((p : ℝ) ^ k) = (1 / ((p : ℝ) ^ 2)) * ((1 / (p : ℝ)) ^ (k - 2)) := by
      have hk_eq : k = (k - 2) + 2 := by omega
      rw [hk_eq, pow_add, div_div]; field_simp [hp_pos.ne'']; ring
    _ ≤ (1 / ((p : ℝ) ^ 2)) * (((1 : ℝ) / 2) ^ (k - 2)) := by
      gcongr
      apply pow_le_pow_of_nonneg_left (by positivity)
      refine (one_div_le_one_div (by positivity) (by norm_num)).mpr ?_
      exact_mod_cast hp
...
lemma geom_tail_Icc_bound (p M : ℕ) (hp : 2 ≤ p) (hM : 2 ≤ M) : 
  ...
  have h_shift : ∑ k ∈ Finset.Icc 2 M, ((1 : ℝ) / 2) ^ (k - 2) = ∑ j ∈ Finset.range (M - 1), ((1 : ℝ) / 2) ^ j := by
    ...
    · intro k hk; apply Finset.mem_range.mpr
      have hk2 := (Finset.mem_Icc.mp hk).1; have hkM := (Finset.mem_Icc.mp hk).2; omega
    · intro k hk; simp
    · intro k1 hk1 k2 hk2 h; omega
    · intro j hj; rw [Finset.mem_range] at hj
      have : j + 2 ∈ Finset.Icc 2 M := Finset.mem_Icc.mpr ⟨by omega, by omega⟩
      refine ⟨j + 2, this, ?_⟩; omega
    · rfl
  calc
    ∑ k ∈ Finset.Icc 2 M, (1 : ℝ) / ((p : ℝ) ^ k)
        ≤ ∑ k ∈ Finset.Icc 2 M, (((1 : ℝ) / 2) ^ (k - 2) * (1 / ((p : ℝ) ^ 2))) :=
      Finset.sum_le_sum (λ k hk => pow_div_pow_bound p hp k ((Finset.mem_Icc.mp hk).1))
    _ = (1 / ((p : ℝ) ^ 2)) * ∑ k ∈ Finset.Icc 2 M, ((1 : ℝ) / 2) ^ (k - 2) := by simp [Finset.mul_sum]
    _ = (1 / ((p : ℝ) ^ 2)) * ∑ j ∈ Finset.range (M - 1), ((1 : ℝ) / 2) ^ j := by rw [h_shift]
    _ ≤ (1 / ((p : ℝ) ^ 2)) * 2 := by gcongr; apply sum_half_pow_range_le_two
    _ = 2 / ((p : ℝ) ^ 2) := by ring

theorem primePower_contribution_bounded :
    ∃ C : ℝ, ∀ x : ℝ, 2 ≤ x →
      |∑ n ∈ Finset.Icc 2 ⌊x⌋₊ with ¬Nat.Prime n,
  ...
  have h_f_nonneg : ∀ p, 0 ≤ f p := by
    intro p; dsimp [f]; split; intro hprime; positivity; rfl
  have h_g_nonneg : ∀ p, 0 ≤ g p := by
    intro p; dsimp [g]; split; intro hprime; positivity; rfl
  have h_g_summable : Summable g := by
    have h_summable_nat : Summable (fun n : ℕ => (n : ℝ) ^ (-3/2 : ℝ)) :=
      (Real.summable_nat_rpow.mpr (by norm_num : (-3/2 : ℝ) < -1))
    have h_indicator : Summable ((Nat.Primes : Set ℕ).indicator (fun n : ℕ => 4 * ((n : ℝ) ^ (-3/2 : ℝ)))) :=
      Summable.indicator (h_summable_nat.const_smul 4) (Nat.Primes : Set ℕ)
    have h_eq : g = (Nat.Primes : Set ℕ).indicator (fun n : ℕ => 4 * ((n : ℝ) ^ (-3/2 : ℝ))) := by
      ext n; simp [g, Set.indicator, Set.mem_setOf_eq]
    rw [h_eq]; exact h_indicator
  have h_f_le_g : ∀ p, f p ≤ g p := by
    intro p; dsimp [f, g]; split <;> try { intro hprime; exfalso; exact hprime }; rfl
    · intro hprime
      have hp2 : 2 ≤ p := Nat.Prime.two_le hprime
      have h_bound : Real.log (p : ℝ) / ((p : ℝ) ^ (2 : ℝ)) ≤ 2 * ((p : ℝ) ^ (-3/2 : ℝ)) := log_div_sq_bound_le p hp2
      calc
        2 * (Real.log (p : ℝ) / ((p : ℝ) ^ (2 : ℝ))) ≤ 2 * (2 * ((p : ℝ) ^ (-3/2 : ℝ))) := by gcongr
        _ = 4 * ((p : ℝ) ^ (-3/2 : ℝ)) := by ring
  have h_f_summable : Summable f := Summable.of_nonneg_of_le h_f_nonneg h_f_le_g h_g_summable
  set C := ∑'' p, f p
  refine ⟨C, λ x hx => ?_⟩
  set N := ⌊x⌋₊
  have hN : 2 ≤ N := by
    have : 2 ≤ x := hx; have : (2 : ℕ) ≤ ⌊x⌋ := by exact_mod_cast (Nat.floor_mono this); omega
  ...
  -- 直接证明: 总合 ≤ ∑_{p素数≤N} f(p)
  -- 对每个素数 p, 其所有幂次 p^k (k≥2) 的总贡献 ≤ f(p)
  have h_total : ∑ n ∈ Finset.Icc 2 N with ¬Nat.Prime n, (vonMangoldt n : ℝ) / (n : ℝ) ≤
      ∑ p ∈ Finset.filter Nat.Prime (Finset.range (N + 1)), f p := by
    -- 构造分组
    let primes : Finset ℕ := Finset.filter Nat.Prime (Finset.range (N + 1))
    have h_group : ∑ n ∈ Finset.Icc 2 N with ¬Nat.Prime n, (vonMangoldt n : ℝ) / (n : ℝ) =
        ∑ p ∈ primes, ∑ n ∈ Finset.filter (λ n : ℕ => ¬Nat.Prime n ∧ Nat.minFac n = p) (Finset.Icc 2 N),
          (vonMangoldt n : ℝ) / (n : ℝ) := by
      have h_disjoint : ∀ p1 p2 ∈ primes, p1 ≠ p2 → Disjoint
          (Finset.filter (λ n : ℕ => ¬Nat.Prime n ∧ Nat.minFac n = p1) (Finset.Icc 2 N))
          (Finset.filter (λ n : ℕ => ¬Nat.Prime n ∧ Nat.minFac n = p2) (Finset.Icc 2 N)) := by
        intro p1 hp1 p2 hp2 hne; apply Finset.disjoint_filter_filter
        intro n hn h; have h1 : Nat.minFac n = p1 := h.2; have h2 : Nat.minFac n = p2 := h.2
        exact hne (h1.trans h2.symm)
      have h_cover : (Finset.Icc 2 N).filter (¬Nat.Prime ·) =
          Finset.biUnion primes (λ p => Finset.filter (λ n : ℕ => ¬Nat.Prime n ∧ Nat.minFac n = p) (Finset.Icc 2 N)) := by
        ext n; constructor
        · intro hn
          have hn_ic := (Finset.mem_filter.mp hn).1; have hn_np : ¬Nat.Prime n := (Finset.mem_filter.mp hn).2
          have hmn2 : n ≠ 0 := by have : 2 ≤ n := (Finset.mem_Icc.mp hn_ic).1; omega
          have hprime_min : Nat.Prime (Nat.minFac n) := Nat.minFac_prime hmn2
          have hp_mem : Nat.minFac n ∈ primes := by
            apply Finset.mem_filter.mpr
            refine ⟨Finset.mem_range.mpr (by
              have hmin_le_n : Nat.minFac n ≤ n := Nat.minFac_le_of_dvd hmn2 (Nat.minFac_dvd n); omega), hprime_min⟩
          apply Finset.mem_biUnion.mpr; refine ⟨Nat.minFac n, hp_mem, Finset.mem_filter.mpr ⟨hn_ic, ⟨hn_np, ?_⟩⟩⟩; rfl
        · intro hn; rcases Finset.mem_biUnion.mp hn with ⟨p, hp, hn'⟩
          apply Finset.mem_filter.mpr; exact ⟨(Finset.mem_filter.mp hn').1, (Finset.mem_filter.mp hn').2.1⟩
      rw [Finset.sum_filter, h_cover, Finset.sum_biUnion h_disjoint]
    
    have h_group_bound : ∀ p ∈ primes, ∑ n ∈ Finset.filter (λ n : ℕ => ¬Nat.Prime n ∧ Nat.minFac n = p) (Finset.Icc 2 N),
        (vonMangoldt n : ℝ) / (n : ℝ) ≤ f p := by
      intro p hp
      have hp_prime : Nat.Prime p := (Finset.mem_filter.mp hp).2
      have hp2 : 2 ≤ p := Nat.Prime.two_le hp_prime
      -- K_p = {k ∈ Icc 2 N | p^k ∈ Icc 2 N}, 这是 p^k ≤ N 的指数
      let K_p : Finset ℕ := Finset.filter (λ k : ℕ => p ^ k ∈ Finset.Icc 2 N) (Finset.Icc 2 N)
      have h_pow_sum : ∑ n ∈ Finset.image (λ k : ℕ => p ^ k) K_p, (vonMangoldt n : ℝ) / (n : ℝ) =
            ∑ k ∈ K_p, Real.log (p : ℝ) / ((p : ℝ) ^ k) := by
        -- 用 Finset.sum_image 和 k→p^k 的单射性 (Nat.pow_right_inj)
        have h_image_eq : Finset.image (λ k : ℕ => p ^ k) K_p = Finset.image (λ k : ℕ => p ^ k) K_p := rfl
        calc
          ∑ n ∈ Finset.image (λ k : ℕ => p ^ k) K_p, (vonMangoldt n : ℝ) / (n : ℝ)
              = ∑ k ∈ K_p, (vonMangoldt (p ^ k) : ℝ) / ((p ^ k : ℕ) : ℝ) :=
            Finset.sum_image (λ x hx y hy h => (Nat.pow_right_inj (by
              have : 1 < p := Nat.Prime.one_lt hp_prime; exact this)).mp h)
        ...
        -- Image ⊆ terms_p
        have h_img_sub : Finset.image (λ k : ℕ => p ^ k) K_p ⊆
            Finset.filter (λ n : ℕ => ¬Nat.Prime n ∧ Nat.minFac n = p) (Finset.Icc 2 N) := by
          intro n hn
          ...
          have hp_pow_ge2 : 2 ≤ p ^ k := by
            have hp2 : 2 ≤ p := Nat.Prime.two_le hp_prime
            calc 2 ≤ 2 ^ k := pow_pos (by omega) k; _ ≤ p ^ k := Nat.pow_le_pow_right (by omega) hp2
          apply Finset.mem_filter.mpr
          refine ⟨Finset.mem_Icc.mpr ⟨hp_pow_ge2, h_pow_N⟩, ?_, ?_⟩
          · apply Nat.not_prime_pow (Nat.Prime.two_le hp_prime) hk2
          · have : p ^ k ≠ 0 := pow_pos (Nat.Prime.pos hp_prime) k
            simp [Nat.minFac_pow hp_prime k]
        ...
        have h_comp_zero : ∑ n ∈ (Finset.filter (λ n : ℕ => ¬Nat.Prime n ∧ Nat.minFac n = p) (Finset.Icc 2 N)           Finset.image (λ k : ℕ => p ^ k) K_p), (vonMangoldt n : ℝ) / (n : ℝ) = 0 := by
          ...
          have hn_mem : n ∈ Finset.filter (λ n : ℕ => ¬Nat.Prime n ∧ Nat.minFac n = p) (Finset.Icc 2 N) :=
          ...
          have h_not_ppow : ¬IsPrimePow n := by
            intro h_ppow
            have h_min : Nat.minFac n = p := (Finset.mem_filter.mp hn_mem).2.2
            have h_min' : Nat.minFac n = (Nat.minFac n) := rfl
            have h_min_p : Nat.minFac n = p := h_min
            -- n 是素数幂, 且其最小素因子是 p, 所以 n = p^e 对某 e
            -- 因此 n ∈ image, 矛盾
            ...
            have h_in_K : e ∈ K_p := by
              ...
              · have : 2 ≤ e := by
                  have h_n_nonprime : ¬Nat.Prime n := (Finset.mem_filter.mp hn_mem).2.1
                  by_contra! h; have h_e1 : e = 1 := by omega; subst h_e1; simp at h_eq
                  exact h_n_nonprime (by
                    rw [h_eq]; apply hp_prime)
                omega
              · have h_n_le_N : n ≤ N := (Finset.mem_Icc.mp (Finset.mem_filter.mp hn_mem).1).2
                rw [h_eq] at h_n_le_N; exact h_n_le_N
              · rw [h_eq]; exact (Finset.mem_filter.mp hn_mem).1
            apply hn_not_img; apply Finset.mem_image.mpr; exact ⟨e, h_in_K, h_eq⟩
          have h_vonM_zero : vonMangoldt n = 0 := (vonMangoldt_zero_iff n).mpr h_not_ppow
          simp [h_vonM_zero]
      
        -- 合并: terms_p 的和 = image 的和 + 补集和 = image 的和
        calc
          ∑ n ∈ Finset.filter (λ n : ℕ => ¬Nat.Prime n ∧ Nat.minFac n = p) (Finset.Icc 2 N), (vonMangoldt n : ℝ) / (n : ℝ)
              = ∑ n ∈ Finset.image (λ k : ℕ => p ^ k) K_p, (vonMangoldt n : ℝ) / (n : ℝ) +
                ∑ n ∈ (Finset.filter (λ n : ℕ => ¬Nat.Prime n ∧ Nat.minFac n = p) (Finset.Icc 2 N)                      Finset.image (λ k : ℕ => p ^ k) K_p), (vonMangoldt n : ℝ) / (n : ℝ) := by
            rw [Finset.sum_sdiff h_img_sub]
          ...
          _ ≤ ∑ k ∈ Finset.Icc 2 N, Real.log (p : ℝ) / ((p : ℝ) ^ k) :=
            Finset.sum_le_sum_of_subset (Finset.filter_subset _ _)
          _ = Real.log (p : ℝ) * ∑ k ∈ Finset.Icc 2 N, (1 : ℝ) / ((p : ℝ) ^ k) := by
            simp [Finset.mul_sum, div_eq_mul_inv, mul_comm, mul_left_comm, mul_assoc]
          _ ≤ Real.log (p : ℝ) * (2 / ((p : ℝ) ^ 2)) := by
            gcongr; apply geom_tail_Icc_bound p hp2 N hN
          _ = f p := by
            dsimp [f]; simp [hp_prime]; ring
    calc
      ∑ n ∈ Finset.Icc 2 N with ¬Nat.Prime n, (vonMangoldt n : ℝ) / (n : ℝ)
          = ∑ p ∈ primes, ∑ n ∈ Finset.filter (λ n : ℕ => ¬Nat.Prime n ∧ Nat.minFac n = p) (Finset.Icc 2 N),
              (vonMangoldt n : ℝ) / (n : ℝ) := h_group
      _ ≤ ∑ p ∈ primes, f p := Finset.sum_le_sum (λ p hp => h_group_bound p hp)
  
  calc
    ∑ n ∈ Finset.Icc 2 N with ¬Nat.Prime n, (vonMangoldt n : ℝ) / (n : ℝ)
        ≤ ∑ p ∈ Finset.filter Nat.Prime (Finset.range (N + 1)), f p := h_total
...
theorem mertens_abel_identity (x : ℝ) (hx : 1 ≤ x) :
  ...
  have h_left : ∑ n ∈ Finset.Icc 0 ⌊x⌋₊, (vonMangoldt n : ℝ) / (n : ℝ) =
      ∑ n ∈ Finset.Icc 0 ⌊x⌋₊, ((n : ℝ)⁻¹) * (vonMangoldt n : ℝ) := by
  ...
  have h_right2 : ∫ t in Set.Ioc (1 : ℝ) x, (deriv (λ u : ℝ => u⁻¹) t) * (∑ n ∈ Finset.Icc 0 ⌊t⌋₊, vonMangoldt n) =
      -∫ t in Set.Ioc 1 x, ψ t / (t * t) := by
    have h_deriv : deriv (λ u : ℝ => u⁻¹) = λ t : ℝ => -(t ^ 2)⁻¹ := by ext t; simp [deriv_inv]
